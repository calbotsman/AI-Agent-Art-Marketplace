...
describe("EndlessMoltNFT", function () {
  ...
  describe("Minting", function () {
    ...
    it("Should allow a wallet to self-mint", async function () {
      await nft.connect(agent).mint(agent.address, metadataURI, agent.address);
      expect(await nft.ownerOf(1)).to.equal(agent.address);
    });

    it("Should not allow minting to a different recipient", async function () {
      await expect(
        nft.connect(agent).mint(buyer.address, metadataURI, agent.address)
      ).to.be.revertedWith("Mint recipient must be the caller");
    });

    it("Should not allow minting with a different creator", async function () {
      await expect(
        nft.connect(agent).mint(agent.address, metadataURI, creator.address)
      ).to.be.revertedWith("Creator must be the caller");
    });
    ...
    it("Should increment token counter correctly", async function () {
      ...
      await nft.connect(buyer).mint(buyer.address, metadataURI, buyer.address);
    ...
    it("Should set correct creator", async function () {
      await nft.connect(agent).mint(agent.address, metadataURI, agent.address);
      expect(await nft.creatorOf(1)).to.equal(agent.address);
    });

    it("Should emit NFTMinted event", async function () {
      await expect(
        nft.connect(agent).mint(agent.address, metadataURI, agent.address)
      )
        .to.emit(nft, "NFTMinted")
        .withArgs(1, agent.address, agent.address, metadataURI);
    });

    it("Should not allow minting with empty metadata URI", async function () {
      await expect(
        nft.connect(agent).mint(agent.address, "", agent.address)
      ).to.be.revertedWith("Empty metadata URI");
    });

    it("Should not allow minting to zero address", async function () {
      await expect(
        nft.connect(agent).mint(ethers.ZeroAddress, metadataURI, agent.address)
      ).to.be.revertedWith("Mint recipient must be the caller");
    });

    it("Should not allow minting with zero creator address", async function () {
      await expect(
        nft.connect(agent).mint(agent.address, metadataURI, ethers.ZeroAddress)
      ).to.be.revertedWith("Creator must be the caller");
    });
  });
  ...
  describe("Royalties (ERC2981)", function () {
    ...
    it("Should return correct royalty info (10%)", async function () {
      ...
      expect(receiver).to.equal(agent.address);
  ...
  describe("Token Operations", function () {
    ...
    it("Should allow token transfers", async function () {
      await nft.connect(agent).transferFrom(agent.address, buyer.address, 1);
