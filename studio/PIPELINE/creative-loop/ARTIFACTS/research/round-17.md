...
## Loop Context
- Loop Policy: min=3, target=3, max=4

## Handoff to Strategy
- Strategy must decide on the core visual concept for Sol Vida's packaging that reflects clarity and material truth.
- Strategy must identify key messaging elements for the packaging that avoid generic wellness tropes.
- Strategy must define how the chosen profile (packaging) effectively communicates Sol Vida's brand values.