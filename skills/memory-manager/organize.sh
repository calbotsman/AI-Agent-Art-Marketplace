...
if [ ! -f "$STATE_FILE" ]; then
  "$SKILL_ROOT/memory-manager/init.sh"
fi
...
for file in $FLAT_FILES; do
  ...
  if [[ "$filename" =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2}\.md$ ]]; then
    ...
    echo "     \"$SKILL_ROOT/memory-manager/categorize.sh\" semantic|procedural|episodic \"$filename\""
    echo ""
  fi
done
...
echo "  2. Use \"$SKILL_ROOT/memory-manager/categorize.sh\" to move them properly"
echo "  3. Run \"$SKILL_ROOT/memory-manager/detect.sh\" to check new usage"
echo ""
echo "Example categorization:"
echo "  \"$SKILL_ROOT/memory-manager/categorize.sh\" semantic moltbook memory/legacy/moltbook-notes.md"
