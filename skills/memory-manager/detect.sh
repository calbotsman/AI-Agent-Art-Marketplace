...
if [ ! -f "$STATE_FILE" ]; then
  "$SKILL_ROOT/memory-manager/init.sh" > /dev/null 2>&1
fi
...
if [ "$usage" -ge 85 ]; then
  ...
  echo "Action needed NOW:"
    echo "  1. Run: \"$SKILL_ROOT/memory-manager/snapshot.sh\""
    echo "  2. Run: \"$SKILL_ROOT/memory-manager/organize.sh\""
  ...
  echo "Recommended actions:"
    echo "  1. Run: \"$SKILL_ROOT/memory-manager/organize.sh\""
